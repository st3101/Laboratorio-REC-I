...
void darDeAlta(eAlumno array[],char*pMensaje1,char*pMensaje2,char*pMensajeError,int limite,eCarrera arrayCarrera[],eCurso arrayCurso[],eMateria arrayMateria[])
{
    int i;
    int aux;
    int flag=0;
    for(i=0;i<limite;i++)
    {
        if(array[i].estado==-1)
        {
            ...
            printf("Ingrese entre la carrera 1 y 2: ");
            scanf("%d",&aux);
            arrayCurso[i].idCurso=i;
            arrayCurso[i].idEstudiante=i;
            arrayCurso[i].idMateria=aux;
            arrayCurso[i].estado=1;
            array[i].estado=1;
            array[i].legajo=i;
            i=limite;
            flag=1;
...
void estudiante_ordenarPorString(eAlumno array[],int limite,eCarrera arrayCarrera[],eCurso arrayCurso[],eMateria arrayMateria[])                              //cambiar estudiante
{
    int i, j;
...
void mostrar(eAlumno array[],int limite,eCarrera arrayCarrera[],eCurso arrayCurso[],eMateria arrayMateria[])
{
    int i;
           ...
           for(i=0;i<limite;i++)
        {
                if(array[i].estado==1)
            {
                printf("\nNombre: %s\nApellido: %s\nLegajo: %d\nCarera: %d\nDescripcion: %s\n",array[i].nombre,array[i].apellido,array[i].legajo,arrayCurso[i].idMateria,arrayMateria[i].descripcionMateria);
...
void listar(eAlumno array[],int limite,eCarrera arrayCarrera[],eCurso arrayCurso[],eMateria arrayMateria[])
{
    ...
    for(i=0;i<limite;i++)
    {
        if(strcmp(array[i].nombre,nombreAux)==0)
        {
           printf("\nNombre: %s\nApellido: %s\nLegajo: %d\nCarera: %d\nDescripcion: %s\n",array[i].nombre,array[i].apellido,array[i].legajo,arrayCurso[i].idMateria,arrayMateria[i].descripcionMateria);
           system("pause");
        }
